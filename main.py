...
def register():
    ...
    if form.validate_on_submit():
        ...
        if test:
            flash('An account with this email address already exists. Please login instead.')
            return redirect(url_for('login'))
        else:
            plain_password = form.password.data
            secure_password = generate_password_hash(plain_password, method="pbkdf2:sha256", salt_length=8)
            new_user = User(
                email = email,
...
def show_post(post_id):
    ...
    if form.validate_on_submit():
        if current_user.is_authenticated:
            ...
            flash('You need to be logged in in order to comment posts')
            return redirect(url_for('login'))
